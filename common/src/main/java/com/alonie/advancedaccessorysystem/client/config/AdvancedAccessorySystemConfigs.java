...
public final class AdvancedAccessorySystemConfigs {
    ...
    private static boolean tryInitClothConfig() {
        try {
            ...
            // AutoConfig.register(AccessoryConfig.class, GsonConfigSerializer::new)
            var registerMethod = autoConfigClass.getMethod("register", Class.class,
                    me.shedaniel.autoconfig.serializer.ConfigSerializer.Factory.class);
            // Create factory: GsonConfigSerializer::new
            var factory = (me.shedaniel.autoconfig.serializer.ConfigSerializer.Factory<?>)
                    serializerClass.getDeclaredConstructor().newInstance();
            clothHolder = registerMethod.invoke(null, configClass, factory);

            // Sync from Cloth Config to static fields
            syncFromCloth();
            return true;
        } catch (Exception e) {
            return false;
