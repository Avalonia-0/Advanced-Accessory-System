...
public final class RideStateSyncClient {
    ...
    public static void handleSync(RideStateSyncPayload payload) {
        enqueue(payload);
        applyOrQueue(Minecraft.getInstance(), payload, false);
    }

    public static void onClientTick(Minecraft client) {
        if (client.level == null || RideSyncClientState.isEmpty()) {
            return;
        }
    ...
    private static boolean applyOrQueue(Minecraft client, RideStateSyncPayload payload, boolean pendingSource) {
        if (client.level == null) {
            return false;
        }

        Entity vehicle = client.level.getEntity(payload.vehicleId());
        Entity passenger = client.level.getEntity(payload.passengerId());
